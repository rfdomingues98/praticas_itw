var vm = function () {
  ...
  if (getUrlParameter("name")) {
    self.baseUri(
      "http://192.168.160.58/netflix/api/search/directors?name=" +
        getUrlParameter("name")
    );
  }
  self.displayName = "Directors";
  self.error = ko.observable("");
  self.totalDirectors = ko.observable(1);
  ...
  self.activate = function (id) {
    ...
    ajaxHelper(composedUri, "GET").done(function (data) {
      ...
      if (getUrlParameter("name")) {
        self.directors(data);
        self.totalDirectors(data.length);
        ...
        self.totalDirectors(data.TotalDirectors);
      }
    });
  };
...
$(document).ready(function () {
  ...
  $(".search-box").autocomplete({
    source: function (request, response) {
      $.ajax({
        url: `http://192.168.160.58/netflix/api/Search/Directors?name=${encodeURIComponent(
          $(".search-box").val()
        )}`,
        dataType: "json",
        data: {
    ...
    select: function (event, ui) {
      window.location.href = window.location.origin.concat(
        `/directorDetails.html?id=${ui.item.id}`
      );
    },
  });
});
