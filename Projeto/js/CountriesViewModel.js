var vm = function () {
  ...
  if (getUrlParameter("name")) {
    self.baseUri(
      "http://192.168.160.58/netflix/api/search/countries?name=" +
        getUrlParameter("name")
    );
  }
  self.displayName = "Countries";
  self.error = ko.observable("");
  self.totalCountries = ko.observable(1);
  ...
  self.activate = function (id) {
    ...
    ajaxHelper(composedUri, "GET").done(function (data) {
      ...
      if (getUrlParameter("name")) {
        self.countries(data);
        self.totalCountries(data.length);
        ...
        self.totalCountries(data.TotalCountries);
      }
    });
  };
...
$(document).ready(function () {
  ...
  $(".search-box").autocomplete({
    source: function (request, response) {
      $.ajax({
        url: `http://192.168.160.58/netflix/api/Search/countries?name=${encodeURIComponent(
          $(".search-box").val()
        )}`,
        dataType: "json",
        data: {
    ...
    select: function (event, ui) {
      window.location.href = window.location.origin.concat(
        `/countryDetails.html?id=${ui.item.id}`
      );
    },
  });
});
